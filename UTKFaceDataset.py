...
# root_dir = 'gdrive/My Drive/Colab Notebooks/Unlearning Fairness/'
# %cd 'gdrive/My Drive/Colab Notebooks/Unlearning Fairness/'
...
class UTKFace(Dataset):
  ...
  def __init__(self, image_dir='UTKFace'):
        ...
        self.filenames = [f for f in listdir(image_dir) if 
                      ...
                      (f.split('_')[0] != 26 or bool(random.getrandbits(1)))  # throw away 50% of 26-year-olds (too many)
        ]
        self.images = []
  ...
  def __getitem__(self, index, label='age'):
        ...
        if label == 'age':
          ...
          age = self.ages[index]
          for idx, bin in enumerate(age_bins):
            if age <= bin:
              label = idx
              break
          if label is 'age':
